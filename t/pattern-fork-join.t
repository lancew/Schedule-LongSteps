...

{
    ...
    sub build_first_step {
        ...
        return $self->new_step(
            { what => 'do_one_thing', run_at => DateTime->now() } );
    }
...

{
    ...
    sub build_first_step {
        my ($self) = @_;
        return $self->new_step(
            { what => 'do_fork', run_at => DateTime->now() } );
    }
    ...
    sub do_fork {
        ...
        my $p2
            = $self->longsteps->instantiate_process('YetAnotherProcessClass');

        return $self->new_step(
            {   what   => 'do_join',
                run_at => DateTime->now(),
                state  => { processes => [ $p1->id(), $p2->id() ], %$state }
            }
        );
    }
...
ok( my $process
        = $long_steps->instantiate_process( 'MyProcess', undef, {} ) );
...

{
    ...
    is( $process->state(),
        {   done    => 'joined',
            pstates => [ { from => 'another' }, { from => 'yetanother' } ]
        }
    );
}

...
ok( my $step = $long_steps->instantiate_process(
        'MyProcess', undef, { beef => 'saussage' }
    )
);

ok( my $fakestep = $long_steps->instantiate_process(
        'MyProcess', undef, { beef => 'kdowkdowk' }
    )
);
...
{
    eval { $long_steps->load_process( $fakestep->id() ); };
    like(
        $@,
        qr(Can\'t locate BladiBlabla.pm),
        'load procress throws an error'
    );
...
is( $long_steps->load_process(1234567890),
    undef, 'load_process returns undef if not found' );

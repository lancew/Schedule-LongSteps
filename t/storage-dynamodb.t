...
my @paws_class = (
    'Paws',                 'Paws::Credential::Explicit',
    'Paws::Net::LWPCaller', 'JSON',
    'MIME::Base64',         'Compress::Zlib',
    'Schedule::LongSteps::Storage::DynamoDB'
);

join( '', map { Class::Load::try_load_class($_) ? 'yes' : '' } @paws_class )
    eq join( '', map {'yes'} @paws_class )
...
ok( my $storage = Schedule::LongSteps::Storage::DynamoDB->new(
        {   dynamo_db    => $dynamo_db,
            table_prefix => 'testdeletethis_' . $test_dynamo_db_uuid
        }
    )
);
like( $storage->table_name(),
    qr/^testdeletethis_${test_dynamo_db_uuid}_Schedule_LongSteps_Storage_DynamoDB/
);
...

{
    ok( my $process_id = $storage->create_process(
            { process_class => 'Blabla', what => 'whatever', run_at => $now }
        )->id(),
        "Ok got ID"
    );
...
$storage->create_process(
    {   process_class => 'Blabla',
        what          => 'some_other_thing',
        run_at        => DateTime->now(),
        id            => 'PLEASE_FIDDLE_WITH_ME'
    }
);
...
is( scalar( $storage->retrieve_processes_by_run_id('FIDDLE_RUN_ID') ),
    1, "Ok found one processes with a run_id of FIDDLE_RUN_ID" );

foreach my $step (@steps) {
    is( $step->run_id, 'i_am_a_teapot',
        'prepare_due_process has the supplied run_id' );
    # While we are doing things, any other process would see zero things to do
    ok( !scalar( $storage->prepare_due_processes() ),
        "Preparing steps again whilst they are running give zero steps" );
}

...
BEGIN {
    ...
    plan skip_all => "DateTime::Format::MySQL is required for this test"
        if $@;
...
#
my $test_mysql
    = Test::mysqld->new( my_cnf => { port => Net::EmptyPort::empty_port() } )
...

{
    ...
    __PACKAGE__->load_components(
        qw/InflateColumn::DateTime InflateColumn::Serializer/);
    __PACKAGE__->add_columns(
        id => {
            data_type         => "integer",
            is_auto_increment => 1,
        ...
        what   => { data_type => "varchar", is_nullable => 1, size => 255 },
        status => {
            data_type     => "varchar",
            is_nullable   => 0,
            size          => 50,
        ...
        run_id => { data_type => "varchar", is_nullable => 1, size => 36 },
        state  => {
            data_type        => "text",
            serializer_class => 'JSON',
            is_nullable      => 0,
        },
        error => { data_type => "text", is_nullable => 1 }
    );
    ...
    sub sqlt_deploy_hook {
        ...
        $sqlt_table->add_index(
            name   => 'idx_longprocess_run_id',
            fields => ['run_id']
        );
        $sqlt_table->add_index(
            name   => 'idx_longprocess_run_at',
            fields => ['run_at']
        );
    }
    1;
...

{
    ...
    __PACKAGE__->load_classes(
        { 'MyApp::Schema::Result' => [ 'Process', 'Patient' ] } );
...
#
{
    ...
    has 'schema' =>
        ( is => 'ro', isa => 'DBIx::Class::Schema', required => 1 );

    has 'patient' => ( is => 'ro', lazy_build => 1 );

    sub _build_patient {
        my ($self) = @_;
        return $self->schema()->resultset('Patient')
            ->find( $self->state()->{patient_id} );
    }

    sub build_first_step {
        my ($self) = @_;
        return $self->new_step(
            { what => 'do_first_look', run_at => DateTime->now() } );
    }
    ...
    sub do_first_look {
        ...
        if ( !$patient->looks_cancerous() ) {
            return $self->final_step(
                { state => { %$state, has_cancer => 0 } } );
        }
        return $self->new_step(
            { what => 'do_analyze_more', run_at => DateTime->now() } );
    }
    ...
    sub do_analyze_more {
        ...
        my $state = $self->state();
        my $p1
            = $self->longsteps->instantiate_process( 'AnalyzePatient',
            { schema => $self->schema() },
            {%$state} );
        my $p2
            = $self->longsteps->instantiate_process( 'AnalyzeFamily',
            { schema => $self->schema() },
            {%$state} );
        return $self->new_step(
            {   what   => 'do_synthetize_analyzes',
                run_at => DateTime->now()->add( days => 3 ),
    ...
    sub do_prescribe {
        ...
        {
            return $self->final_step(
                { state => { %$state, have_treatment => 1 } } );
        }
        return $self->final_step(
            { state => { %$state, have_treatment => 0 } } );
    }
...

{
    ...
    sub build_first_step {
        my ($self) = @_;
        return $self->new_step(
            { what => 'do_analyze_patient', run_at => DateTime->now() } );
    }
...

{
    ...
    sub build_first_step {
        my ($self) = @_;
        return $self->new_step(
            {   what   => 'do_analyze_family',
                run_at => DateTime->now()->add( days => 2 )
            }
        );
    }
    ...
    sub do_analyze_family {
        ...
        return $self->final_step(
            {   state => {
                    has_cancer_history => $self->patient->family_history()
                }
            }
        );
    }
...
my $long_steps_dbixclass
    = Schedule::LongSteps->new( { storage => $storage_dbixclass } );
push @longsteps, $long_steps_dbixclass;

my $storage_auto = Schedule::LongSteps::Storage::AutoDBIx->new(
    { get_dbh => sub { return $schema->storage()->dbh(); } } );
my $long_steps_auto
    = Schedule::LongSteps->new( { storage => $storage_auto } );
...
if ( $ENV{DYNAMODB_LOCAL} ) {
    ...
    $dynamo_storage = Schedule::LongSteps::Storage::DynamoDB->new(
        { dynamo_db => $dynamo_db, table_prefix => 'testdeletethis' } );
    $dynamo_storage->vivify_table();

    my $long_steps_dynamo
        = Schedule::LongSteps->new( { storage => $dynamo_storage } );
...
foreach my $long_steps (@longsteps) {
    ...
    my $patient
        = $schema->resultset('Patient')->create( { name => 'Joe Foobar' } );
    my $cancerous_patient = $schema->resultset('Patient')
        ->create( { name => 'Joe BarBaz', looks_cancerous => 1 } );
    my $cancerous_family
        = $schema->resultset('Patient')
        ->create(
        { name => 'Joe BarBaz', looks_cancerous => 1, family_history => 1 } );

    ok( my $healthy_process = $long_steps->instantiate_process(
            'MyMedicalProcess',
            { schema     => $schema },
            { patient_id => $patient->id() }
        )
    );
    ok( my $cancer_process = $long_steps->instantiate_process(
            'MyMedicalProcess',
            { schema     => $schema },
            { patient_id => $cancerous_patient->id() }
        )
    );
    ok( my $family_process = $long_steps->instantiate_process(
            'MyMedicalProcess',
            { schema     => $schema },
            { patient_id => $cancerous_family->id() }
        )
    );
    ...
    $healthy_process->discard_changes()
        ;    # This is needed, cause the framework only does 'update'
...
END {
    if ($dynamo_storage) {
        if ( $dynamo_storage->table_exists() ) {
            $dynamo_storage->destroy_table(
                'I am very sure and I am not insane');
        }
    }
    done_testing();
}

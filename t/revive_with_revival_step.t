...
like(
    $@,
    qr(Unable revive \d+ to do_the_hoff),
    'revive to an incorrect function'
);

is( $long_steps->revive( $process->id() ),
    1, 'Process was revived, but will still fail' );
ok( $long_steps->run_due_processes(), 'run the revival step' );
like(
    $process->error(),
    qr(something went wrong),
    'Although rivived this is still broken '
);

is( $long_steps->revive( $process->id(), {}, 'revive_do_break_stuff' ),
    1, 'Process was revived with its revival step' );
...
my $revival_methods
    = $long_steps->load_process( $process->id() )->revival_methods();
is( scalar(@$revival_methods), 1, 'there is only one revival method' );
is( $revival_methods->[0], 'revive_do_break_stuff',
    'the only revival methodis revive_do_break_stuff' );
done_testing();

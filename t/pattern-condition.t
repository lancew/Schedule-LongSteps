...

{
    ...
    sub build_first_step {
        ...
        return $self->new_step(
            { what => 'do_choose', run_at => DateTime->now() } );
    }
    ...
    sub do_choose {
        ...
        if ( $self->state()->{choice} == 1 ) {
            return $self->new_step(
                { what => 'do_choice1', run_at => DateTime->now() } );
        }
        return $self->new_step(
            { what => 'do_choice2', run_at => DateTime->now() } );
    }

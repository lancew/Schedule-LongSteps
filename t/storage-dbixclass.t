...
ok( my $storage = Schedule::LongSteps::Storage::DBIxClass->new(
        { schema => $schema, resultset_name => 'LongstepsProcess' }
    )
);
...
my @processes_with_supplied_run_id
    = $storage->prepare_due_processes( { run_id => 'i_am_a_teapot' } );
is( scalar(@processes_with_supplied_run_id), 1, "Ok one due step" );
is( $processes_with_supplied_run_id[0]->run_id,
    'i_am_a_teapot', 'prepare_due_process has the supplied run_id' );
...

{
    ...
    ok( $storage->find_process( $process->id() ) );
    throws_ok {
        $storage->update_process( $process, { run_at => DateTime->now() } )
    }
    qr/unmanaged exception/;
    is( $times_called, 1,
        "Ok called only one time, because it died with something unmanaged" );
}
